...
### D19. Concurrency scope and re-evaluation
- Parallel mutation tests must invoke real production handlers and assert persisted-state invariants, not infer safety from sequential version injection alone. In the currently tested SQLite topology, overlapping document revisions and connection mutations produced one successful advancement/transition and one 409 conflict without duplicate or partial logical rows. No distributed or multi-writer guarantees are claimed. Re-evaluate concurrency guarantees if the production database or deployment topology changes.
